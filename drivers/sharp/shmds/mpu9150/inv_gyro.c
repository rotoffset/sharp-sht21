...
static int shmds_buf_size = 50;
int shmds_buf_size_temp = 50;
...
static void regulator_ctrl(struct inv_gyro_state_s *st, unsigned int flag)
{
	...
	msdrv_vreg_8921_l9 = regulator_get(st->inv_dev, "8921_l9");
	
	if(flag == SHMDS_REG_CHG_LPM) {
		ret = regulator_set_mode(msdrv_vreg_8921_l9, REGULATOR_MODE_STANDBY);
	}
	else if(flag == SHMDS_REG_CHG_NORMAL) {
		ret = regulator_set_mode(msdrv_vreg_8921_l9, REGULATOR_MODE_NORMAL);
	}
...
/**
 * inv_pedometer_clip_threshold_store() -  calling this function will
 *                        store current clip threshold into MPU memory
 */
static ssize_t inv_pedometer_clip_threshold_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data, out;
...
/**
 * inv_pedometer_min_steps_store() -  calling this function will
 *                        store current min. steps into MPU memory
 */
static ssize_t inv_pedometer_min_steps_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data;
	unsigned short sdata, out;
	unsigned char *p;
	if (st->chip_config.is_asleep)
		return -EPERM;
	result = kstrtoul(buf, 10, (long unsigned int *)&data);
...
/**
 * inv_pedometer_max_step_buffer_time_store() -  calling this function will
 *                        store current max. step buffer time into MPU memory
 */
static ssize_t inv_pedometer_max_step_buffer_time_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data;
...
/**
 * inv_pedometer_threshold_store() -  calling this function will
 *                        store current threshold into MPU memory
 */
static ssize_t inv_pedometer_threshold_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data, out;
...
/**
 * inv_pedometer_min_up_time_store() -  calling this function will
 *                        store current min. up time into MPU memory
 */
static ssize_t inv_pedometer_min_up_time_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data;
...
/**
 * inv_pedometer_max_up_time_store() -  calling this function will
 *                        store current max. up time into MPU memory
 */
static ssize_t inv_pedometer_max_up_time_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data;
...
/**
 * inv_pedometer_min_energy_store() -  calling this function will
 *                        store current min. energy into MPU memory
 */
static ssize_t inv_pedometer_min_energy_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	unsigned int result, data, out;
...
/**
 * inv_pedometer_clip_threshold_show() -  calling this function will read
 *                        current pedometer clip threshold from MPU memory
 */
static ssize_t inv_pedometer_clip_threshold_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result, data;
...
/**
 * inv_pedometer_min_steps_show() -  calling this function will read
 *                        current min. steps from MPU memory
 */
static ssize_t inv_pedometer_min_steps_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result;
	...
	if (result)
		return result;
	data = be16_to_cpup((unsigned short *)d);
	return sprintf(buf, "%d\n", data);
}

/**
 * inv_pedometer_max_step_buffer_time_show() -  calling this function will
 *                        read current max. step buffer time from MPU memory
 */
static ssize_t inv_pedometer_max_step_buffer_time_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result;
...
/**
 * inv_pedometer_threshold_show() -  calling this function will
 *                        read current threshold from MPU memory
 */
static ssize_t inv_pedometer_threshold_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result, data;
...
/**
 * inv_pedometer_min_up_time_show() -  calling this function will
 *                        read current min. up time from MPU memory
 */
static ssize_t inv_pedometer_min_up_time_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result;
...
/**
 * inv_pedometer_max_up_time_show() -  calling this function will
 *                        read current max. up time from MPU memory
 */
static ssize_t inv_pedometer_max_up_time_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result;
...
/**
 * inv_pedometer_min_energy_show() -  calling this function will
 *                        read current min. energy from MPU memory
 */
static ssize_t inv_pedometer_min_energy_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	int result, data;
...
/* shmds add -> */
	if (st->trigger.irq == MSM_GPIO_TO_INT(GPIO_GYRO_INT)) {
...
static void timerhandler_work(struct work_struct *work)
{
	...
	else if (st->trigger.irq == MSM_GPIO_TO_INT(GPIO_GYRO_INT)) {
#else
	if (st->trigger.irq == MSM_GPIO_TO_INT(GPIO_GYRO_INT)) {
#endif /* SHMDS_DETECT */
...
static DEVICE_ATTR(pedometer_clip_threshold, 0664,
	inv_pedometer_clip_threshold_show, inv_pedometer_clip_threshold_store);
static DEVICE_ATTR(pedometer_min_steps, 0664,
	inv_pedometer_min_steps_show, inv_pedometer_min_steps_store);
static DEVICE_ATTR(pedometer_max_step_buffer_time, 0664,
	inv_pedometer_max_step_buffer_time_show,
	inv_pedometer_max_step_buffer_time_store);
static DEVICE_ATTR(pedometer_threshold, 0664,
	inv_pedometer_threshold_show, inv_pedometer_threshold_store);
static DEVICE_ATTR(pedometer_min_up_time, 0664,
	inv_pedometer_min_up_time_show, inv_pedometer_min_up_time_store);
static DEVICE_ATTR(pedometer_max_up_time, 0664,
	inv_pedometer_max_up_time_show, inv_pedometer_max_up_time_store);
static DEVICE_ATTR(pedometer_min_energy, 0664,
	inv_pedometer_min_energy_show, inv_pedometer_min_energy_store);
...
static struct device_attribute *inv_mpu6050_attributes[] = {
	...
	&dev_attr_pedometer_clip_threshold,
	&dev_attr_pedometer_min_steps,
	&dev_attr_pedometer_max_step_buffer_time,
	&dev_attr_pedometer_threshold,
	&dev_attr_pedometer_min_up_time,
	&dev_attr_pedometer_max_up_time,
	&dev_attr_pedometer_min_energy,
/* shmds add -> */
...
/* shmds add -> */
	if (client->irq == MSM_GPIO_TO_INT(GPIO_GYRO_INT)) {
