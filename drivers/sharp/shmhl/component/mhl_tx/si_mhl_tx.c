...
#ifndef CONFIG_SII8334_MHL_TX /* Delete because of Not Use */
...
#ifdef CONFIG_SII8334_MHL_TX
				TX_DEBUG_PRINT(("MHL_MSC_MSG_RCP [0x%x]\n", mhlTxConfig.mscMsgData));
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
                    mhlTxConfig.mscLastRetryCount = pReq->retryCount;
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
				shmhl_notify_si_state(SHMHL_SI_STATE_HAVE_DEV_CAT, &data1);
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
bool_t shmhl_retry_send_msc_command(void)
{
    cbus_req_t	req;
    uint8_t		ccode;
    
    CBUS_DEBUG_PRINT (("MhlTx:%s start\n", __func__));
    
    if (!mhlTxConfig.mscLastCommand)
    {
        CBUS_DEBUG_PRINT (("MhlTx:%s no last command!\n", __func__));
        return false;
    }
    
    if (mhlTxConfig.mscLastRetryCount > 0)
    {
        CBUS_DEBUG_PRINT (("MhlTx:%s retry remain:%d\n", __func__, mhlTxConfig.mscLastRetryCount));
        
        req.retryCount            = --(mhlTxConfig.mscLastRetryCount);
        req.command               = mhlTxConfig.mscLastCommand;
        if (MHL_MSC_MSG == req.command)
        {
            req.payload_u.msgData[0]  = mhlTxConfig.mscMsgLastCommand;
            req.payload_u.msgData[1]  = mhlTxConfig.mscMsgLastData;
        }
        else
        {
            req.offsetData            = mhlTxConfig.mscLastOffset;
            req.payload_u.msgData[0]  = mhlTxConfig.mscLastData;
        }
        ccode = PutPriorityCBusTransaction(&req);
    }
    else
    {
        CBUS_DEBUG_PRINT (("MhlTx:%s retry over!\n", __func__));
        ccode = false;
    }
