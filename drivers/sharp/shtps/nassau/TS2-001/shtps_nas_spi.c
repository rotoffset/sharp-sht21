...
#define SHTPS_STARTUP_RETRY_MAX				5
...
static void shtps_check_fw_startup(struct shtps_nas_spi *ts)
{
	...
	if(ts->fw_update_continue == 0x00){
		if(buf == 0x00){
			...
			if(buf != 0xFF){
				ts->fw_startup_state = SHTPS_FW_STARTUP_ERROR;
			}else{
				ts->startup_retry_count = 0;
...
static int shtps_statef_resetting_int(struct shtps_nas_spi *ts, int param)
{
	...
	if(intsrc == SHTPS_NAS_INT_NONE){
		...
		if(ts->fw_startup_state == SHTPS_FW_STARTUP_ERROR){
			if(ts->startup_retry_count++ < SHTPS_STARTUP_RETRY_MAX){
				shtps_statef_cmn_reset_recovery(ts, ts->pending);
			}
		}else if(ts->fw_startup_state == SHTPS_FW_STARTUP_PROGRAM){
