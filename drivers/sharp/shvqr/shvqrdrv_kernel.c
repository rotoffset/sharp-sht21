	...
	if(t && t->mm && t->mm->mmap)
	{
		...
		for(i = 0; i < t->mm->map_count; i++)
		{
			...
			if(file)
			{
				...
				p = d_path(&file->f_path, tmp, PAGE_SIZE - 1);
...
static int shvqrdrv_open(struct inode* inode, struct file* filp)
{
	...
	do
	{
		...
		if(shvqrdrv_guess_binary(current, buff, PAGE_SIZE - 1) == NULL)
		{
			ret = -EPERM;
		...
		if(strlen(buff) != strlen(SHVQRDEV_CHECK_PATH))
		{
			ret = -EPERM;
		...
		if(strcmp(buff, SHVQRDEV_CHECK_PATH) != 0)
		{
			ret = -EPERM;
