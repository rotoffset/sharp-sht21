...
static char ragdata_es1[REGISTER_MAX] = {0x00,0x00,0x00,0xFF,0xFF,0xFF,0x0F,0x0A,0x50,0xB7,0x65,0x0A,0x58,0x6A,0x10,0x9C,0x0F,0x1C,0x10,0x11,0x00};
static char ragdata_pp1[REGISTER_MAX] = {0x00,0x00,0x00,0xFF,0xFF,0xFF,0x0F,0x0A,0x50,0xB7,0x65,0x0A,0x58,0x6A,0x10,0x9C,0x0F,0x1C,0x10,0x11,0x00};
static char ragdata_pp2[REGISTER_MAX] = {0x00,0x00,0x00,0xFF,0xFF,0xFF,0x0F,0x0A,0x50,0xB7,0x65,0x0A,0x58,0x6A,0x10,0x9C,0x0F,0x1C,0x10,0x11,0x00};
static char ragdata_other[REGISTER_MAX]={0x00,0x00,0x00,0xFF,0xFF,0xFF,0x0F,0x0A,0x50,0xB7,0x65,0x0A,0x58,0x6A,0x10,0x9C,0x0F,0x1C,0x10,0x11,0x00};
...
static void SHGEIGER_DBUGPRINT(struct shgeiger_count_data *data)
{
	DEBUG_SENSORDATA_LOG("time_slot %d num_atom %d num_dc %d thermal %d,total_time_nsec %llu,total_stop_time_nsec %llu",data->num_time_slot,data->num_atom,data->num_dc,data->thermal_data,data->total_time_nsec,data->total_stoptime_nsec);
}
...
//	nResult = gpio_request(SH_GEIGER_EN,"SH_GEIGER_EN");
//	if (nResult != 0){
//		DEBUG_ERROR_LOG("GEIGER_EN Active Setting Error");
//		goto error;
//	}
...
static int IOECS_AlarmWait( void __user *argp )
{
	...
	wait_event_freezable(alarmwait_t,(alarm_wait != ALARM_ALREADY_LOCKED));
...
static int IOECS_ThermalWait( void __user *argp )
{
	...
	wait_event_freezable(thermalwait_t,(thermal_wait != THERMAL_ALREADY_LOCKED));
...
static int IOECS_GMBIST( void __user *argp )
{
	...
	nResult = SHGEIGER_I2cWrite(this_client,0x0D,0x6E);
	if (nResult != 0){
		DEBUG_ERROR_LOG("I2cWrite Error");
		nResult_flag = SH_GEIGER_RESULT_FAILURE;
	}
...
static int SHGEIGER_Probe(I2cClt *client, const I2cDevID *poDevId)
{
	...
	if(nResult < 0){
		DEBUG_ERROR_LOG("SHGEIGER_CalilbrationRead Error");
		nResult = SH_GEIGER_RESULT_FAILURE;
		goto exit2;
	}
