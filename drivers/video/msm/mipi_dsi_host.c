...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00025 */ /* CUST_ID_00110 */
static int err_contention_lp1_flag = 0;
static int dsi_clk_cnt = 0;
#endif
...
const int cpm_qos_latency_WFI=0;
const int cpm_qos_latency_POWER_COLLAPSE_STANDALONE=1300;
const int cpm_qos_latency_POWER_COLLAPSE=2000;
...
{
	pm_qos_update_request(&mipi_dsi_qos_req, -1);
}
#endif /* CONFIG_SHLCDC_BOARD */
...
{
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00110 */
...
int mipi_dsi_clk_cfg_check(void)
{
	return dsi_clk_cnt;
}

void mipi_dsi_clk_cfg_clear(void)
{
	mutex_lock(&clk_mutex);
	dsi_clk_cnt = 0;
	mutex_unlock(&clk_mutex);
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00025 */ /* CUST_ID_00087 */ /* CUST_ID_00088 */
void mipi_dsi_set_err_int_mask0_b25(void)
{
    uint32 tmp_val;
    tmp_val = MIPI_INP(MIPI_DSI_BASE + 0x0108);/* DSI_ERR_INT_MASK0 */
    tmp_val &= ~0x02000000;
    MIPI_OUTP(MIPI_DSI_BASE + 0x0108, tmp_val); /* DSI_ERR_INT_MASK0 */
    err_contention_lp1_flag = 0;
}

void mipi_dsi_clr_err_int_mask0_b25(void)
{
    uint32 tmp_val;
    tmp_val = MIPI_INP(MIPI_DSI_BASE + 0x0108);/* DSI_ERR_INT_MASK0 */
    tmp_val |= 0x02000000;
    MIPI_OUTP(MIPI_DSI_BASE + 0x0108, tmp_val); /* DSI_ERR_INT_MASK0 */
    err_contention_lp1_flag = 3;
}

int mipi_dsi_get_err_contention_lp1_flag(void)
{
    return err_contention_lp1_flag;
}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00025 */ /* CUST_ID_00074 */ /* CUST_ID_00087 */ /* CUST_ID_00088 */
	if (pinfo->mode == DSI_CMD_MODE) {
		MIPI_OUTP(MIPI_DSI_BASE + 0x0108, 0x11ff3fe0); /* DSI_ERR_INT_MASK0 */
			if(mipi_sharp_is_recovery() == 0) {
				err_contention_lp1_flag = 0;
				mipi_dsi_det_recovery_flag_clr();
			}
	}
	else {
...
void mipi_dsi_clkin_force_ctl(int enable)
{
	u32 tmp;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00118 */
#ifndef CONFIG_FB_MSM_MIPI_SHARP_FLUTE_VIDEO_FWVGA_PT_PANEL
	mipi_dsi_clk_cfg(1);
	...
	*/
	...
	for (i = 0; i < cnt; i++) {
		...
		if (cm->wait)
			...
			mipi_sharp_delay_us(cm->wait*1000);
...
static void mipi_dsi_cmd_dma_tx_err(struct dsi_buf *tp)
{
	if (tp->len <= 0) {
		pr_err("dma_tx: len=%d\n", tp->len);
	} else if (tp->len == 1) {
		pr_err("dma_tx: len=%d, data=0x%02x\n", tp->len, tp->data[0]);
	} else {
		pr_err("dma_tx: len=%d, data=0x%02x,0x%02x\n", tp->len, tp->data[0], tp->data[1]);
	}
}
#endif /* CONFIG_SHLCDC_BOARD */
...
int mipi_dsi_cmd_dma_tx(struct dsi_buf *tp)
{
	...
	int_cnt = mdp4_stat.intr_dsi_cmd;
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00096 */
	wait_ret = wait_for_completion_timeout(&dsi_dma_comp, msecs_to_jiffies(10000));
	if (wait_ret <= 0) {
		pr_err("dsi_clk=%d->%d, mdp_clk=%d->%d, isr=%d, count=%lu->%lu\n",
				dsi_clk_info, mipi_dsi_clk_on, mdp_clk_info, mdp_get_clk_cnt(), mdp_is_in_isr, int_cnt, mdp4_stat.intr_dsi_cmd);
		pr_err("dsi_dma_comp=0x%lx\n", (unsigned long)dsi_dma_comp.done);
		mdp_req_trace_dump(0x01);
		mipi_dsi_cmd_dma_tx_err(tp);
...
void mipi_dsi_cmd_mdp_busy(void)
{
	...
	dsi_clk_info = mipi_dsi_clk_on;
	mdp_clk_info = mdp_get_clk_cnt();
	int_cnt = mdp4_stat.intr_dsi_mdp;
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00096 */
	if (need_wait) {
		wait_ret = wait_for_completion_timeout(&dsi_mdp_comp, msecs_to_jiffies(10000));
		if (wait_ret <= 0) {
			pr_err("dsi_clk=%d->%d, mdp_clk=%d->%d, isr=%d, count=%lu->%lu\n",
					dsi_clk_info, mipi_dsi_clk_on, mdp_clk_info, mdp_get_clk_cnt(), mdp_is_in_isr, int_cnt, mdp4_stat.intr_dsi_mdp);
			pr_err("dsi_mdp_comp=0x%lx\n", (unsigned long)dsi_mdp_comp.done);
			mdp_req_trace_dump(0x01);
			WARN(1, "%s: timeout Err!\n", __func__);
			panic("%s: timeout Err!\n", __func__);
		}
...
void mipi_dsi_dln0_phy_err(void)
{
	...
	if (status & 0x011111) {
        ...
        if (status & 0x010000) {
            ...
            tmp_val |= 0x02000000;
            MIPI_OUTP(MIPI_DSI_BASE + 0x0108, tmp_val); /* DSI_ERR_INT_MASK0 */
            if (err_contention_lp1_flag == 0) {
                err_contention_lp1_flag = 1;
            }
        }
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00034 */
void mipi_dsi_pll_lock_status(void)
{
	uint32 status;

	status = MIPI_INP(MIPI_DSI_BASE + 0x011c);/* DSI_CLK_STATUS */

	if (status & 0x00010000) {
		MIPI_OUTP(MIPI_DSI_BASE + 0x011c, 0x00010000);
		pr_debug("%s: status=%x\n", __func__, status);
	}
}
#endif /* CONFIG_SHLCDC_BOARD */
...
irqreturn_t mipi_dsi_isr(int irq, void *ptr)
{
	...
	if (isr & DSI_INTR_ERROR) {
		...
		mipi_dsi_error();
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00025 */
        if(err_contention_lp1_flag == 1) {
            err_contention_lp1_flag = 2;
            mipi_dsi_det_recovery();
        }
#endif
	}
