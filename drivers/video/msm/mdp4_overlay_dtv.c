...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00048 */ /* CUST_ID_00096 */
...
static void mdp4_dtv_vsync_ctrl_db_out(void)
{
	struct vsycn_ctrl *vctrl;
	vctrl = &vsync_ctrl_db[0];

	pr_err("[ vsycn_ctrl ] inited=%d,update_ndx=%d,dmae_intr_cnt=%d\n", 
			vctrl->inited,
			vctrl->update_ndx,
			vctrl->dmae_intr_cnt
	);
	pr_err("[ ---------- ] suspend=%d,vsync_resume=%d,dmae_wait_cnt=%d,wait_vsync_cnt=%d,blt_change=%d,sysfs_created=%d\n", 
			(int)atomic_read(&vctrl->suspend),
			(int)atomic_read(&vctrl->vsync_resume),
			vctrl->dmae_wait_cnt,
			vctrl->wait_vsync_cnt,
			vctrl->blt_change,
			vctrl->sysfs_created
	);
	pr_err("[ ---------- ] ov_comp=0x%lx,dmae_comp=0x%lx,vsync_comp=0x%lx\n", 
			(unsigned long)vctrl->ov_comp.done,
			(unsigned long)vctrl->dmae_comp.done,
			(unsigned long)vctrl->vsync_comp.done
	);
	pr_err("[ ---------- ] vsync_irq_enabled=%d\n", 
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00150 */
	if (!mdp4_dtv_hdmi_is_open()) {
		pr_debug("%s : open canceled\n", __func__);
		return 0;
	}
#endif /* CONFIG_SHLCDC_BOARD */
...
void mdp4_dtv_wait4vsync(int cndx, long long *vtime)
{
	...
	int_cnt = mdp4_stat.intr_vsync_e;
#endif /* CONFIG_SHLCDC_BOARD */
	...
	spin_unlock_irqrestore(&vctrl->spin_lock, flags);
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00096 */
	wait_ret = wait_for_completion_timeout(&vctrl->vsync_comp, msecs_to_jiffies(10000));
	if (wait_ret <= 0) {
		pr_err("%s %d  TIMEOUT_\n", __func__, __LINE__);
		pr_err("dsi_clk=%d->%d, mdp_clk=%d->%d, isr=%d, count=%lu->%lu\n",
				dsi_clk_info, mipi_dsi_clk_on, mdp_clk_info, mdp_get_clk_cnt(), mdp_is_in_isr, int_cnt, mdp4_stat.intr_vsync_e);
		mdp_clk_info_out();
		mdp4_dtv_vsync_ctrl_db_out();
...
static void mdp4_dtv_wait4ov(int cndx)
{
	...
	dsi_clk_info = mipi_dsi_clk_on;
	mdp_clk_info = mdp_get_clk_cnt();
	int_cnt = mdp4_stat.intr_overlay1;
	wait_ret = wait_for_completion_timeout(&vctrl->ov_comp, msecs_to_jiffies(10000));
	if (wait_ret <= 0) {
		pr_err("%s %d  TIMEOUT_\n", __func__, __LINE__);
		pr_err("dsi_clk=%d->%d, mdp_clk=%d->%d, isr=%d, count=%lu->%lu\n",
				dsi_clk_info, mipi_dsi_clk_on, mdp_clk_info, mdp_get_clk_cnt(), mdp_is_in_isr, int_cnt, mdp4_stat.intr_overlay1);
		mdp_clk_info_out();
		mdp4_dtv_vsync_ctrl_db_out();
		mdp_req_trace_dump(0x1F);
		WARN(1, "%s: timeout Err!\n", __func__);
...
static void mdp4_dtv_wait4dmae(int cndx)
{
	...
	dsi_clk_info = mipi_dsi_clk_on;
	mdp_clk_info = mdp_get_clk_cnt();
	int_cnt = mdp4_stat.intr_dma_e;
	wait_ret = wait_for_completion_timeout(&vctrl->dmae_comp, msecs_to_jiffies(10000));
	if (wait_ret <= 0) {
		pr_err("%s %d  TIMEOUT_\n", __func__, __LINE__);
		pr_err("dsi_clk=%d->%d, mdp_clk=%d->%d, isr=%d, count=%lu->%lu\n",
				dsi_clk_info, mipi_dsi_clk_on, mdp_clk_info, mdp_get_clk_cnt(), mdp_is_in_isr, int_cnt, mdp4_stat.intr_dma_e);
		mdp_clk_info_out();
		mdp4_dtv_vsync_ctrl_db_out();
		mdp_req_trace_dump(0x1F);
		WARN(1, "%s: timeout Err!\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = wait_for_completion_interruptible_timeout(&vctrl->vsync_comp,
		msecs_to_jiffies(VSYNC_PERIOD * 4 * 2));
#else
	ret = wait_for_completion_interruptible_timeout(&vctrl->vsync_comp,
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00152 */
	memset(&vctrl->vlist[0], 0, sizeof(vctrl->vlist));
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00048 */ /* CUST_ID_00150 */
	if ((vctrl->vsync_irq_enabled) && (mdp4_dtv_hdmi_is_open())){
...
void mdp4_dtv_wait_flush(void)
{
	...
	if (mdp4_dtv_hdmi_is_open()) {
		for (i = 0; i < 34; i++) {
			...
			reg = inpdw(MDP_BASE + 0x18000);
			pr_debug("%s i:%02d/33, reg:0x%08x\n", __func__, i, reg);
			if ((reg & 2) == 0)
				break;
		}
	} else {
		mipi_sharp_delay_us(16666);
	}
	return;
}
#endif /* CONFIG_SHLCDC_BOARD */
