...
static inline struct sock *l2cap_get_sock_by_alarm(struct alarm *alarm)
{
	struct sock *sk = NULL;
...
static void l2cap_sock_timeout_handler(struct alarm *alarm)
{
	struct sock *sk = NULL;
	
	BT_DBG("");
	
	sk = l2cap_get_sock_by_alarm(alarm);
	
	if (sk) {
		BT_DBG("sk = %p", sk);
		l2cap_sock_timeout((unsigned long)sk);
	}
}

static void l2cap_sock_set_alarm(struct sock *sk, unsigned int msecs)
{
	long secs = msecs / 1000;
	unsigned long nsecs = (msecs % 1000) * 1000;
	ktime_t expires = ktime_add(alarm_get_elapsed_realtime(), ktime_set( secs, nsecs ));
	
	BT_DBG("sk = %p, secs = %ld, nsecs = %lu", sk, secs, nsecs);
	
	alarm_start_range(&l2cap_pi(sk)->sk_alarm, expires, expires);
	sock_hold(sk);
}

static void l2cap_sock_clear_alarm(struct sock *sk)
{
	BT_DBG("sk = %p", sk);
	if (alarm_try_to_cancel(&l2cap_pi(sk)->sk_alarm) == 1) {
		__sock_put(sk);
	}
}
...
#ifdef BLUETOOTH_CUSTOMIZE
	l2cap_sock_set_alarm(sk, jiffies_to_msecs(timeout));
...
#ifdef BLUETOOTH_CUSTOMIZE
		if ((l2cap_pi(sk)->psm == cpu_to_le16(0x0017))
		||  (l2cap_pi(sk)->psm == cpu_to_le16(0x0019))) {
			release_sock(sk);
			return 0;
		}
#endif /* BLUETOOTH_CUSTOMIZE */
...
#ifdef BLUETOOTH_CUSTOMIZE
	alarm_init(&l2cap_pi(sk)->sk_alarm, ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP, l2cap_sock_timeout_handler);
#else /* BLUETOOTH_CUSTOMIZE */
