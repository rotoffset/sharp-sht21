					...
					u16 latency, u16 to_multiplier)
{
	...
	BT_DBG("");
	set_bit(HCI_CONN_LE_CONN_UPDATE_PEND, &conn->pend);
	if (test_bit(HCI_CONN_LE_START_ENC_PEND, &conn->pend)) {
		BT_DBG("HCI_CONN_LE_START_ENC_PEND, defer the request");
		return;
	}
...
#ifdef BLUETOOTH_CUSTOMIZE
	set_bit(HCI_CONN_LE_START_ENC_PEND, &conn->pend);
	if (test_bit(HCI_CONN_LE_CONN_UPDATE_PEND, &conn->pend)) {
		memcpy(&conn->cp, &cp, sizeof(cp));
		BT_DBG("HCI_CONN_LE_CONN_UPDATE_PEND, defer the request");
		return;
	} else
		memset(&conn->cp, 0, sizeof(cp));

	BT_DBG("send HCI LE Start Encryption");
#endif /* BLUETOOTH_CUSTOMIZE */
...
#ifdef BLUETOOTH_CUSTOMIZE
static inline struct hci_conn *hci_conn_hash_lookup_alarm(struct alarm *alarm)
{
	struct list_head *p_dev, *p_conn;
	struct hci_conn  *conn = NULL;
	
	read_lock(&hci_dev_list_lock);
	list_for_each(p_dev, &hci_dev_list) {
		struct hci_dev *hdev = list_entry(p_dev, struct hci_dev, list);
		struct hci_conn_hash *conn_hash = &hdev->conn_hash;
		
		list_for_each(p_conn, &conn_hash->list) {
			conn = list_entry(p_conn, struct hci_conn, list);
			
			if (&conn->disc_alarm == alarm) {
...
static void hci_conn_timeout_handler(struct alarm *alarm)
{
	struct hci_conn  *conn = NULL;
	
	BT_DBG("");

	conn = hci_conn_hash_lookup_alarm(alarm);
	
	if (conn) {
		BT_DBG("conn = %p", conn);
		hci_conn_timeout((unsigned long)conn);
	}
}

void hci_set_disc_alarm(struct hci_conn *conn, unsigned int msecs)
{
	long secs = msecs / 1000;
	unsigned long nsecs = (msecs % 1000) * 1000;
	ktime_t expires = ktime_add(alarm_get_elapsed_realtime(), ktime_set( secs, nsecs ));

	BT_DBG("conn = %p, secs = %ld, nsecs = %lu", conn, secs, nsecs);

	alarm_start_range(&conn->disc_alarm, expires, expires);
}

void hci_clear_disc_alarm(struct hci_conn *conn)
{
	BT_DBG("conn = %p", conn);
	alarm_try_to_cancel(&conn->disc_alarm);
}
#endif /* BLUETOOTH_CUSTOMIZE */
...
#ifdef BLUETOOTH_CUSTOMIZE
	alarm_init(&conn->disc_alarm, ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP, hci_conn_timeout_handler);
