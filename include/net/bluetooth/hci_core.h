...
/* BLUETOOTH_CUSTOMIZE Start */
	struct hci_cp_le_start_enc cp;
/* BLUETOOTH_CUSTOMIZE End */
#ifdef BLUETOOTH_CUSTOMIZE
	struct alarm disc_alarm;
#endif /* BLUETOOTH_CUSTOMIZE */
...
/* BLUETOOTH_CUSTOMIZE Start */
	HCI_CONN_LE_CONN_UPDATE_PEND,
	HCI_CONN_LE_START_ENC_PEND,
/* BLUETOOTH_CUSTOMIZE End */
};
...
#ifdef BLUETOOTH_CUSTOMIZE
void hci_set_disc_alarm(struct hci_conn *conn, unsigned int msec);
...
static inline void hci_conn_put(struct hci_conn *conn)
{
	if (atomic_dec_and_test(&conn->refcnt)) {
		...
		if (conn->type == ACL_LINK || conn->type == LE_LINK) {
			...
			timeo = msecs_to_jiffies(10);
#ifdef BLUETOOTH_CUSTOMIZE
		hci_set_disc_alarm(conn, jiffies_to_msecs(timeo));
#else /* BLUETOOTH_CUSTOMIZE */
