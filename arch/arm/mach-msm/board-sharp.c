...
#if defined(CONFIG_SHTPS_NASSAU_DEV)
#include <sharp/shtps_dev_nassau.h>
#endif /* #if defined(CONFIG_SHTPS_NASSAU_DEV) */

#ifdef CONFIG_MPU_SENSORS_MPU9150
#include <sharp/mpu9150.h>
#endif /* CONFIG_MPU_SENSORS_MPU9150 */
...
{
    int ret;
    ret = gpio_request(SHTPS_SY3X00_GPIO_IRQ, "shtps_irq");
    if(ret != 0){
        return ret;
    }

    ret = gpio_request(SHTPS_SY3X00_GPIO_RST, "shtps_rst");
    if(ret != 0){
        return ret;
    }
...
#ifdef CONFIG_SHTPS_NASSAU_TS2_001
static int shtps_nassau_touch_gpio_setup(struct device *dev)
{
	int rc;
	
    rc = gpio_request(SHTPS_NASSAU_GPIO_IRQ, SPI_TOUCH_NAME"_gpio_irq");
    if (rc)	printk(KERN_DEBUG "%s: unable to request SHTPS_NASSAU_GPIO_IRQ [%d]\n", __func__, SHTPS_NASSAU_GPIO_IRQ);

    rc = gpio_direction_input(SHTPS_NASSAU_GPIO_IRQ);
    if (rc)	printk(KERN_DEBUG "%s: unable to set_direction SHTPS_NASSAU_GPIO_IRQ [%d]\n", __func__, SHTPS_NASSAU_GPIO_IRQ);

    rc = gpio_request(SHTPS_NASSAU_GPIO_RST, SPI_TOUCH_NAME"_gpio_rst");
    if (rc)	printk(KERN_DEBUG "%s: unable to request SHTPS_NASSAU_GPIO_RST [%d]\n", __func__, SHTPS_NASSAU_GPIO_RST);

    rc = gpio_direction_output(SHTPS_NASSAU_GPIO_RST, 0);
    if (rc)	printk(KERN_DEBUG "%s: unable to set_direction SHTPS_NASSAU_GPIO_RST [%d]\n", __func__, SHTPS_NASSAU_GPIO_RST);

    rc = gpio_request(PM8921_GPIO_PM_TO_SYS(SHTPS_NASSAU_GPIO_PWR), SPI_TOUCH_NAME"_gpio_pwr");
    if (rc)	printk(KERN_DEBUG "%s: unable to request SHTPS_NASSAU_GPIO_PWR [%d]\n", __func__, SHTPS_NASSAU_GPIO_PWR);

    rc = gpio_request(SHTPS_NASSAU_GPIO_OSC, SPI_TOUCH_NAME"_gpio_osc");
    if (rc)	printk(KERN_DEBUG "%s: unable to request SHTPS_NASSAU_GPIO_OSC [%d]\n", __func__, SHTPS_NASSAU_GPIO_OSC);

    rc = gpio_direction_output(SHTPS_NASSAU_GPIO_OSC, 0);
    if (rc)	printk(KERN_DEBUG "%s: unable to set_direction SHTPS_NASSAU_GPIO_OSC [%d]\n", __func__, SHTPS_NASSAU_GPIO_OSC);

	{
		...
		rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(SHTPS_NASSAU_GPIO_PWR), &param);
		if (rc < 0) {
			pr_err("failed to configure TP_PWR_EN gpio\n");
			gpio_free(PM8921_GPIO_PM_TO_SYS(SHTPS_NASSAU_GPIO_PWR));
			return rc;
		}
	}
...
static void shtps_nassau_touch_gpio_teardown(struct device *dev)
{
    gpio_free(SHTPS_NASSAU_GPIO_IRQ);
    gpio_free(SHTPS_NASSAU_GPIO_RST);
    gpio_free(PM8921_GPIO_PM_TO_SYS(SHTPS_NASSAU_GPIO_PWR));
    gpio_free(SHTPS_NASSAU_GPIO_OSC);
}

static int shtps_nassau_VL17_power(int on)
{
    ...
    if(!reg_8921_l17){
        ...
        reg_8921_l17 = regulator_get(&msm8960_device_qup_spi_gsbi9.dev, "shtps_nas_vdda");
        if (IS_ERR(reg_8921_l17)) {
            pr_err("could not get reg_8921_l17, ret = %ld\n",
                PTR_ERR(reg_8921_l17));
            return -ENODEV;
        }
        
        pr_info("%s: regulator_set_voltage(reg_8921_l17)\n", __func__);
        ret = regulator_set_voltage(reg_8921_l17, 1800000, 1800000);
        if (ret) {
            pr_err("set_voltage failed for 8921_l17, ret=%d\n", ret);
            return -EINVAL;
        }
    }
...
static struct shtps_nassau_touch_platform_data shtps_nassau_touch_data = {
    .setup    			= shtps_nassau_touch_gpio_setup,
    .teardown			= shtps_nassau_touch_gpio_teardown,
    .v17_power          = shtps_nassau_VL17_power,
	.gpio_irq_pin		= SHTPS_NASSAU_GPIO_IRQ,
	.gpio_reset_pin		= SHTPS_NASSAU_GPIO_RST,
	.gpio_power_pin		= PM8921_GPIO_PM_TO_SYS(SHTPS_NASSAU_GPIO_PWR),
	.gpio_oscstart_pin	= SHTPS_NASSAU_GPIO_OSC,
	.x_min				= 0,
	.x_max				= 0x2600,
	.y_min				= 0,
	.y_max				= 0x1800,
};
#endif /* #ifdef CONFIG_SHTPS_NASSAU_TS2_001 */
...
#ifdef CONFIG_SHTPS_NASSAU_TS2_001
	{
		.modalias		= SPI_TOUCH_NAME,
        .irq			= MSM_GPIO_TO_INT(SHTPS_NASSAU_GPIO_IRQ),
		.max_speed_hz	= SHTPS_NASSAU_SPI_CLOCK,
		.bus_num		= 9,
		.chip_select	= 0,
		.mode			= SPI_MODE_3,
		.platform_data	= &shtps_nassau_touch_data,
	},
#endif /* #ifdef CONFIG_SHTPS_NASSAU_TS2_001 */
};
...
static struct platform_device msm_device_chg = {
    .name = "shchg",
};
...
#endif	/* CONFIG_SHFELICA */
#ifdef	CONFIG_SH_PHS_CUST
	&msm8960_device_uart_gsbi4,
	&msm8960_device_uart_gsbi5,
#endif	/* CONFIG_SH_PHS_CUST */
...
/* ADD_S shmds GSBI12 from DSPS to Krait */
    ...
    mb();   //Make sure the data is completely written

    //Disable the clk if it was disabled earlier
    if (!ClkStatus) {
...
#ifdef CONFIG_SENSORS_SHGEIGER
static struct i2c_board_info shgeiger_i2c_info[] __initdata = {
    {
...
static void __init register_i2c_devices(void)
{
	...
	if (machine_is_msm8960_cdp())
		...
		mach_mask = I2C_FFA;
	else if (machine_is_tori())
		mach_mask = I2C_SURF;
	else if (machine_is_manabe())
		mach_mask = I2C_SURF;
	else if (machine_is_arisa())
		mach_mask = I2C_SURF;
	else if (machine_is_deckard_as70())
		mach_mask = I2C_SURF;
	else if (machine_is_deckard_gp4())
		mach_mask = I2C_SURF;
	else if (machine_is_dsn14())
		mach_mask = I2C_SURF;
	else
...
static void __init msm8960_cdp_init(void)
{
	...
	if (machine_is_msm8960_mtp() || machine_is_msm8960_fluid() ||
		machine_is_msm8960_cdp() || machine_is_tori() || machine_is_manabe() || machine_is_arisa() || machine_is_deckard_as70() || machine_is_deckard_gp4() || machine_is_dsn14()) {
	...
	if (machine_is_msm8960_cdp() || machine_is_tori() || machine_is_manabe() || machine_is_arisa() || machine_is_deckard_as70() || machine_is_deckard_gp4() || machine_is_dsn14())
...
MACHINE_START(DECKARD_AS70, "SHARP DECKARD AS70")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
...
MACHINE_START(DECKARD_GP4, "SHARP DECKARD GP4")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
...
MACHINE_START(DSN14, "SHARP DSN14")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
